...
int byteSend;
int data[2][9]; 
// af uransagelige orsager skal den være på 9 selv om der kun bruges 8 !!
...
void loop()
 {
  newData = false;
	...
	RecvWithEndMarker(); // modtager data til \n
	 // udtræk data fra buffer og gem i Array
	if (newData == true){FindDataIBufferX(0);}
	...
	RecvWithEndMarker(); // modtager data til \n
   // udtræk data fra buffer og gem i Array
	if (newData == true){	FindDataIBufferX(1);} 
	digitalWrite(Pin13LED, LOW);  // Show activity
...
void FindDataIBufferX(int NX)       // afhængig af slaven
{
 ...
 for (int i=0; i <= 8; i++)
  {
   //Finder positionen for de 8 bogstaver i XA & gemmer pos i Value[]
