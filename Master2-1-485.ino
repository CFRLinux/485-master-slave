 ...
 for (int i=0; i <= 8; i++)
  {
   if (value[i] >= 0 )
     {
      //udtræk talværderne der står mellem bugstaverne & gem i StringTo
      StringTo = StringOne.substring(value[i]+1, value[i+1]);	 
      data[NX][i]=StringTo.toInt(); // kanvater data i StringTo til INT og gem i data[NX][i]
     }
   else
    {
     data[NX][i]=0;
